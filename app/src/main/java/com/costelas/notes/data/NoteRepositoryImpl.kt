...
class NoteRepositoryImpl(
    ...
    override fun getAllNotes(): Flow<Result<List<Note>>> = callbackFlow {
        val listener = userCollection.document(auth.uid!!).collection("com.costelas.notes").apply {  }
    ...
    override suspend fun addNote(note: Note): Result<Boolean> {
        return try {
            userCollection.document(auth.uid!!)
                .collection("com.costelas.notes").document(note.uid).set(note)
            Result.Success(true)
        } catch (e: Exception) {
            e.printStackTrace()
